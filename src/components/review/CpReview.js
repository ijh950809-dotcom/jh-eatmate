...
const CpReview = ({ mypageUser, mypageCategory }) => {
  ...
  const loadData = async () => {
    try {
      ...
      if (!mypageUser) {
        // 메인 - 맛집 리뷰 목록
        res = await axios.get('https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/review/all');
      } else {
        // 마이페이지 - 작성한 게시글 - 맛집 리뷰 목록
        res = await axios.get('https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/review', {
            ...
            <li key={item.br_no}>
              <Link to={`/review/detail/${item.br_no}`}>
                <div className="img-box">
                  <img src={`https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/uploads/review/${item.br_img}`} alt={`${item.rt_name} 사진`} />
                </div>

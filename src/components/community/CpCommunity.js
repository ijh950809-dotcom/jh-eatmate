...
const CpCommunity = ({ mypageUser, mypageCategory }) => {
  ...
  const loadData = () => {
    if (!mypageUser) {
      ...
      // 마이페이지 - 작성한 게시글
      axios.get('https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/community', {
            ...
            <Link to={`/community/detail/${item.bc_no}`} className='item'>
              ...
              <div className="comm-top">
                ...
                <div className="img-box">
                  <img src={`https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/uploads/user/${item.u_pic}`} alt={`${item.u_nick} 프로필`} />
                </div>
                {/* 닉네임 + 등록시간 */}
                <p className='txt-box'>

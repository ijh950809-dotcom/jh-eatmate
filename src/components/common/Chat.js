import { useEffect, useMemo, useState } from 'react';
import axios from 'axios';
import './Chat.scss';
import Comment from './Chat/Comment';
import InputComment from './Chat/InputComment';
import { dateFormat } from 'utils/dateFormat';
import { jwtDecode } from 'jwt-decode';

function Chat({ p_board_cate, p_board_no, onRefreshBoard }) {
  const [chatDate, setChatDate] = useState([]);
  const token = localStorage.getItem('token');
  const decoded = token ? jwtDecode(token) : '';
  const loginUserNo = decoded?.token_no;

  // props로 받아와서 backend로 넘김
  const boardInfo = useMemo(() => ({
    board_cate: p_board_cate, // board 카테고리(맛집 리뷰, 맛집 탐방, 자유게시판)
    board_no: p_board_no, // board 게시판 번호
  }), [p_board_cate, p_board_no]);


  const loadData = async () => {
    try {
      const res = await axios.post('https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/common/chat', boardInfo);

      setChatDate(res.data);
    } catch (err) {
      console.log(err.response)
    }
  }

  useEffect(() => {
    loadData();
  }, [p_board_cate, p_board_no])

  const handleDelete = async (ct_no) => {
    if (window.confirm('댓글을 삭제하시겠습니까?')) {
      try {
        await axios.delete(`https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/chat/comment/${ct_no}`, {
          data: boardInfo,
        });

        alert('댓글이 삭제되었습니다.');
        loadData(); // 댓글 목록 즉시 갱신
        onRefreshBoard?.(); // 게시글(댓글 수) 즉시 갱신
      } catch (err) {
        console.log(err);
      }
    }
  }

  return (
    <div className='common-chat'>
      <InputComment
        p_board_cate={p_board_cate}
        p_board_no={p_board_no}
        onAfterSubmit={() => {
          loadData();
          onRefreshBoard?.();
        }}
      />
      <ul className='comm-comment'>
        {
          chatDate.map(item => {
            const isMine = loginUserNo === item.ct_user_no;

            return (
              <Comment
                key={item.ct_no}
                profil={`${item.u_pic}`}
                nick={`${item.u_nick}`}
                txt={`${item.ct_desc}`}
                time={`${dateFormat(item.ct_date)}`}
                isMine={isMine}
                onDelete={() => handleDelete(item.ct_no)}
              />
            )
          })
        }
      </ul>
    </div>
  );
}

export default Chat;
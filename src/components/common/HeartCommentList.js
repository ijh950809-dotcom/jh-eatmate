...
const HeartComment = ({ heart, comment, p_board_cate, p_board_no, p_user_token }) => {
  ...
  const hasHeartClick = async () => {
    try {
      ...
      const res = await axios.post(`https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/heart/user`, payload);
  ...
  const heartClick = async () => {
    ...
    try {
      ...
      await axios.put(`https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/board/heart`, payload);

      // heard 테이블의 데이터 입력(생성)
      await axios.post(`https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/heart`, payload);
    } catch (err) {
      console.log(err);

...
function RestaurantList(props) {
  ...
  const loadData = async () => {
    try {
      const res = await axios.post('https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/restaurant');
  ...
  const deleteData = async (rt_no, rt_name) => {
    if (window.confirm(`${rt_name}을(를) 삭제하시겠습니까?`)) {
      try {
        await axios
          .delete(`https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/admin/restaurant/${rt_no}`);

...
function MeetupList(props) {
  ...
  const loadData = async () => {
    try {
      const res = await axios.get('https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/meetup/all');
      setData(res.data);
    } catch (err) {
  ...
  const deleteData = async (bm_no, u_nick) => {
    if (window.confirm(`${u_nick}님의 게시글을 삭제하시겠습니까?`)) {
      try {
        await axios.delete(`https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/admin/meetup/${bm_no}`);
              ...
              <tbody>
                {data.map(item => (
                  <tr key={item.bm_no}>
                    ...
                    <td>{item.bm_title}</td>
                    <td>{item.bm_desc}</td>
                    <td className='imgtd'><img src={`https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/uploads/meetup/${item.bm_img}`} alt="탐방 사진" ></img></td>
                    <td>{item.bm_m_res}</td>

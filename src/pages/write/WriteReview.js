...
const WriteReview = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/write/review/modify/${br_no}`)
      .then(res => {
        setReviewInput(prev => ({
          ...prev,
          br_user_no: decoded.token_no,
  ...
  const rtSearch = async (word) => {
    ...
    try {
      const res = await axios.post('https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/restaurant/search', { word });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!br_no) {
        // 등록하기
        const res = await axios.post('https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/write/review', formData);
        alert('맛집 리뷰 글쓰기가 완료되었습니다. 작성한 게시글로 이동합니다.');
        navigate(`/review/detail/${res.data.br_no}`);
      } else {
        // 수정하기
        await axios.put(`https://port-0-jh-eatmate-backend-mleqh0x837c33d90.sel3.cloudtype.app/write/review/modify/${br_no}`, formData);
